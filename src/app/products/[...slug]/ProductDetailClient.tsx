  ...
  if (!product) {
    return (
      <div className="pt-32 pb-20 text-center min-h-[60vh] flex flex-col items-center justify-center bg-slate-50">
        ...
        <Link href="/products" className="mt-6 px-6 py-3 bg-sky-600 text-white rounded-xl text-xs font-semibold">
    ...
    <div className="pt-24 min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 py-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-2 text-xs text-slate-600 font-medium overflow-x-auto whitespace-nowrap">
          ...
          <Link href="/products" className="hover:text-sky-700 transition-colors shrink-0">
            Products
          </Link>
          <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
          <Link
            href={`/products/${product.slug}`}
            className={`hover:text-sky-700 transition-colors shrink-0 ${!activeSubcategory ? "font-bold text-slate-900" : ""}`}
          >
            {product.name}
          </Link>
          {activeSubcategory && (
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-b from-white via-sky-50/60 to-sky-100/50 py-12 lg:py-16 border-b border-sky-100">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-7 space-y-4">
              ...
              {isCAR && (
                <div className="flex flex-wrap gap-3 pt-2">
                  <div className="px-4 py-2 bg-white rounded-xl border border-sky-200 shadow-sm font-mono">
                    ...
                    <span className="text-sm font-extrabold text-sky-900">50–200 dia</span>
                  </div>
                  <div className="px-4 py-2 bg-white rounded-xl border border-sky-200 shadow-sm font-mono">
              ...
              {isFireCanvas && activeSubcategory?.temperature && (
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-sky-900 text-white rounded-xl shadow-md font-mono text-xs">
              ...
              {product.standards && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {product.standards.map((std) => (
                    <span
                      key={std}
                      className="inline-flex items-center gap-1.5 text-xs font-mono font-bold px-3 py-1 rounded-md bg-sky-50 text-sky-800 border border-sky-200"
                    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-14">
        ...
        {subcategories.length > 0 && (
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-sky-200 shadow-lg space-y-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span>{sub.name}</span>
                      {sub.model && (
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono ${isSelected ? "bg-sky-900 text-sky-200" : "bg-sky-100 text-sky-800"}`}>
                          {sub.model}
                        </span>
                      )}
                      {sub.temperature && (
            ...
            {activeSubcategory && (
              <div className="p-5 bg-sky-50/50 rounded-2xl border border-sky-100 text-slate-700 space-y-2 mt-4">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {product.models.map((mod) => (
                        <tr key={mod.name} className="hover:bg-slate-50 transition-colors">
                          <td className="p-3 font-bold text-sky-900">{mod.name}</td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {PRODUCTS.filter((p) => p.id !== product.id).map((rel) => (
              <Link
                ...
                className="group bg-white p-4 rounded-2xl border border-slate-200 hover:border-sky-300 shadow-xs hover:shadow-md transition-all flex items-center gap-3"
              >
